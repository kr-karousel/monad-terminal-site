...
async function _syncNicknamesFromServer(){
  if(!_sbClient) return;
  try{
    const { data } = await _sbClient.from('nicknames').select('address, nickname').or('terminal.eq.bob,terminal.is.null');
    if(!data) return;
    data.forEach(row => {
...
function _subscribeToNicknames(){
  if(!_sbClient) return;
  _sbClient.channel('sync-nicknames-bob')
    .on('postgres_changes',
      { event: '*', schema: 'public', table: 'nicknames', filter: 'terminal=eq.bob' },
      payload => {
        const row = payload.new;
        if(!row || !row.address || !row.nickname) return;
        if(row.terminal && row.terminal !== 'bob') return;
...
async function syncNickToServer(address, nickname){
  ...
  try{
    await _sbClient.from('nicknames').upsert(
      { address: address.toLowerCase(), nickname, terminal: 'bob', updated_at: new Date().toISOString() },
...
async function _syncShoutsFromServer(){
  ...
  try{
    const { data } = await _sbClient
      ...
      .or('terminal.eq.bob,terminal.is.null')
...
function _subscribeToShouts(){
  if(!_sbClient) return;
  _sbClient.channel('sync-shouts-bob')
    .on('postgres_changes',
      { event: 'INSERT', schema: 'public', table: 'shouts' },
      payload => {
        const row = payload.new;
        if(!row) return;
        // BOB 터미널 shout만 처리 (terminal='bob' 또는 NULL=기존 데이터)
        if(row.terminal && row.terminal !== 'bob') return;
        const entry = { addr: row.nickname || row.address, msg: row.message, id: row.id };
...
async function syncShoutToServer(address, nickname, message){
  if(!_sbClient) return;
  try{
    await _sbClient.from('shouts').insert({ address: address.toLowerCase(), nickname, message, terminal: 'bob' });
...
async function _syncMessagesFromServer(){
  if(!_sbClient) return;
  try{
    // bob_bal IS NOT NULL → BOB 터미널에서 보낸 메시지만 로드
    const { data } = await _sbClient
      .from('messages')
      .select('id, address, nickname, content, created_at, bob_bal')
      .not('bob_bal', 'is', null)
      .order('created_at', { ascending: false })
      .limit(10);
    if(!data || !data.length) return;
...
function _subscribeToMessages(){
  if(!_sbClient) return;
  // 'sync-messages-bob' 채널: BOB 터미널 전용 (bob_bal 기준 필터)
  _sbClient.channel('sync-messages-bob')
    .on('postgres_changes',
      { event: 'INSERT', schema: 'public', table: 'messages' },
      payload => {
        const row = payload.new;
        if(!row) return;
        // bob_bal이 없으면 MON 터미널 메시지 → 무시
        if(row.bob_bal === null || row.bob_bal === undefined) return;
        renderMsg({
          addr: row.address,
          addrFull: row.address,
          nickname: row.nickname || null,
...
async function syncMessageToServer(address, nickname, content, bobBal){
  if(!_sbClient) return;
  try{

...
const CONTRIB_KEY = 'bob_contrib_v1'; // localStorage fallback
const REV_TOP_N   = 20;
...
async function renderRevenueModal() {
  ...
  if (balEl)  balEl.textContent  = devBal.toLocaleString(undefined, { maximumFractionDigits:0 }) + ' BOB';
  if (poolEl) poolEl.textContent = poolAmt.toLocaleString(undefined, { maximumFractionDigits:0 }) + ' BOB';
...
function renderContribTable(devBal, serverContribs) {
  ...
  top.forEach((e, idx) => {
    ...
    rows += `<tr class="${rowCls}">
      ...
      <td class="rev-td rev-tr rev-est">${est!=null?'~'+Math.floor(est).toLocaleString()+' BOB':'—'}</td>
    </tr>`;
  });
...
async function devDistributeRevenue() {
  ...
  const preview = top.map((e, i) => {
    ...
    return `#${i+1} ${nick} → ${amt.toLocaleString()} BOB (${pct}%)`;
  }).join('\n');

  if (!confirm(`Distribute ${Math.floor(poolAmt).toLocaleString()} BOB to ${top.length} contributors?\n\n${preview}\n\nThis will send ${top.length} transactions.`)) return;

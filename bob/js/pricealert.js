...
function loadPriceAlerts(){
  try{ priceAlerts = JSON.parse(localStorage.getItem('bob_price_alerts')||'[]'); }
  catch(e){ priceAlerts = []; }
  alertEmail = localStorage.getItem('bob_alert_email') || '';
  updateAlertBellState();
}

function savePriceAlerts(){
  localStorage.setItem('bob_price_alerts', JSON.stringify(priceAlerts));
}
...
async function saveAlertEmail(){
  ...
  alertEmail = val;
  localStorage.setItem('bob_alert_email', val);
...
function _fireAlert(alert, currentPrice){
  ...
  const msg = `BOB hit $${alert.price.toFixed(7)}\nNow: $${currentPrice.toFixed(7)}`;

  // Browser notification
  if(alertNotifGranted && 'Notification' in window){
    try{ new Notification('🎯 Terminal Price Alert', { body: msg, icon: '/bob/img/bob_logo.png' }); }
    catch(e){}
  }

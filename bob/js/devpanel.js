...
var devShowTradePhoto = localStorage.getItem('bob_show_photo') !== 'off';

function toggleTradePhoto(){
  devShowTradePhoto = !devShowTradePhoto;
  localStorage.setItem('bob_show_photo', devShowTradePhoto ? 'on' : 'off');
...
async function devResetChessRecords(){
  ...
  try{
    const db = JSON.parse(localStorage.getItem('bob_contrib_v1')||'{}');
    ...
    localStorage.setItem('bob_contrib_v1', JSON.stringify(db));
...
function updateCostDisplays(){
  ...
  if(shoutDisp) shoutDisp.textContent = '💜 ' + SHOUT_COST.toLocaleString() + ' BOB';
  const shoutHeader = document.getElementById('shoutHeaderCost');
  if(shoutHeader) shoutHeader.textContent = SHOUT_COST.toLocaleString() + ' BOB';
  const nickDisp = document.getElementById('nickCostDisplay');
  if(nickDisp) nickDisp.textContent = '💜 ' + NICK_COST.toLocaleString() + ' BOB';
}

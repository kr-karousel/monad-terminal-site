  ...
  for(const url of urls){
    try{
      ...
      d.items.forEach(item => {
        ...
        const bob  = Number(BigInt(raw.toString().replace('.','').split('.')[0]||'0')) / 1e18;
        const ts    = item.timestamp ? Math.floor(new Date(item.timestamp).getTime()/1000) : 0;
        if(bobAmt <= 0 || ts < cutoff) return;

        const isBuy  = from === poolL || from === routerL;
        const isSell = to   === poolL || to   === routerL;
        if(isBuy)       trades.push({ type:'buy',  bob:bobAmt, block:0, txHash: item.tx_hash, time: ts });
        else if(isSell) trades.push({ type:'sell', bob:bobAmt, block:0, txHash: item.tx_hash, time: ts });
      });
...
async function fetchWalletTrades(addr, maxBlocks){
  ...
  try {
    ...
    // null wildcard: all BOB transfers TO user + all FROM user
    ...
    (inLogs||[]).forEach(log => {
      ...
      const bob  = Number(BigInt('0x' + log.data.slice(2))) / 1e18;
  ...
  const recentHTML = trades.slice(0, 15).map(t => {
    ...
    return `
      <div style="display:flex;align-items:center;gap:6px;padding:5px 0;border-bottom:1px solid rgba(255,255,255,.04)">
        ...
        <span style="font-size:10px;font-family:'Share Tech Mono',monospace;flex:1">${amt} BOB</span>
  ...
  return `
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px">
      <div style="background:rgba(255,255,255,.04);border-radius:10px;padding:10px;text-align:center">
        ...
        <div style="font-size:10px;color:var(--muted)">BOB · $${totalVal.toFixed(2)}</div>
      </div>
      <div style="background:rgba(255,255,255,.04);border-radius:10px;padding:10px;text-align:center">
      ...
      <div style="background:rgba(34,197,94,.06);border:1px solid rgba(34,197,94,.15);border-radius:10px;padding:10px;text-align:center">
        ...
        <div style="font-size:9px;color:var(--muted)">BOB</div>
      </div>

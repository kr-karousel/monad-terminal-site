...
const NADFUN_POOL   = '0x116e7D070f1888B81E1E0324F56d6746B2D7d8f1'; // Capricorn V3 BOB/WMON
...
const TOTAL_SUPPLY  = 1_000_000_000; // 1B BOB total supply
...
var NICK_BANNED  = [
  ...
  'official','support','scam','bob','bob_official','nadfun',
...
// ═══════════════════════════════════════
const RANKS=[
  {min:100000000,label:'BOB GOD',badge:'👑 GOD',cls:'r1'},
  {min:10000000,label:'Dragon Overlord',badge:'🐉 DRAGON',cls:'r2'},
  {min:1000000,label:'BOB Emperor',badge:'👸 EMPEROR',cls:'r3'},
  ...
  {min:0,label:'ZeroBOB Ghost',badge:'👻 GHOST',cls:'r10'},
];
function getRank(b, addr){
  // DEV 지갑은 항상 최상위 계급으로 표시
  if(addr && addr.toLowerCase() === DEV_WALLET.toLowerCase()){
    return {min:0,label:'BOB Terminal DEV',badge:'🛠️ DEV',cls:'r1',isDev:true};
  }
  // 커스텀 Tier 라벨 확인
  if(addr){
...
function updateChogPosDirect(pY){
  try{
    const char    = document.getElementById('bobChar');
...
function handleSwapLog(log) {
  try {
    ...
    showTradeFloat(isBuy, usdValue, bobAmount, monAmount);

    // 채팅창 거래 알림 (1000 MON 이상)
    if(monAmount >= 1000) {
      const txHash = log.transactionHash || '';
      const _isBuy = isBuy, _bobAmount = bobAmount, _priceUsd = priceUsd, _monAmount = monAmount;
      ...
      bobEmotion(isBuy ? 'buy' : 'sell');
    }

    console.log(isBuy?'🟢 BUY':'🔴 SELL', bobAmount.toFixed(0),'BOB | $'+usdValue.toFixed(2),'| $'+priceUsd.toFixed(8));
...
function openNickModal(){
  ...
  body.innerHTML = `
    ...
    <div class="nick-cost">
      ...
      <span class="nick-cost-badge" id="nickCostDisplay">💜 2,000 BOB</span>
    </div>
    ...
    if(typeof chessLoadStats === 'function'){
      chessLoadStats(addrFull).then(stats => {
        ...
        if(stats){
          ...
          rEl.textContent  = total > 0 ? rate + '%' : '—';
          ptEl.textContent = stats.pts || 0;
        } else {
          wEl.textContent = lEl.textContent = rEl.textContent = ptEl.textContent = '0';
        }
      });
    }
  }
...
async function sendBobTo(toAddr){
  if(!wallet){ alert('Connect your wallet first!'); return; }
...
function switchSwapSide(side){
  ...
  if(titleEl){
    titleEl.textContent = isBuy ? '▲ BUY BOB' : '▼ SELL BOB';
    titleEl.style.color = isBuy ? 'var(--green)' : 'var(--red)';
  }

  // From/To labels & icons
  document.getElementById('swapFromLabel').textContent = isBuy ? 'You Pay (MON)' : 'You Pay (BOB)';
  document.getElementById('swapToLabel').textContent   = isBuy ? 'You Receive (BOB)' : 'You Receive (MON)';
  document.getElementById('swapFromName').textContent  = isBuy ? 'MON' : 'BOB';
  document.getElementById('swapToName').textContent    = isBuy ? 'BOB' : 'MON';

  // Icons - SVG 인라인 방식
  const isBuyMode = side === 'buy';
  // FROM: BUY=MON, SELL=BOB
  document.getElementById('svgMonFrom').style.display  = isBuyMode ? 'block' : 'none';
  document.getElementById('svgBobFrom').style.display = isBuyMode ? 'none'  : 'block';
  // TO: BUY=BOB, SELL=MON
  document.getElementById('svgBobTo').style.display   = isBuyMode ? 'block' : 'none';
  document.getElementById('svgMonTo').style.display    = isBuyMode ? 'none'  : 'block';

  // Execute button
  const execBtn = document.getElementById('btnSwapExec');
  if(execBtn){
    execBtn.textContent = isBuy ? '▲ BUY BOB' : '▼ SELL BOB';
...
function swapUpdateBalances(){
  ...
  if(fromEl) fromEl.textContent = isBuy
    ...
    : `Balance: ${(wallet.bal||0).toLocaleString()} BOB`;
...
async function swapCalcOutLens(amtIn){
  ...
  try{
    ...
    if(result && result !== '0x' && result.length >= 66){
      ...
      if(isBuy){
        ...
        document.getElementById('swapPriceVal').textContent = `1 MON ≈ ${rate.toLocaleString(undefined,{maximumFractionDigits:0})} BOB`;
      } else {
        const rate = outAmt / amtIn;
        document.getElementById('swapPriceVal').textContent = `1 BOB ≈ ${rate.toFixed(6)} MON`;
      }
      return;
    }
  } catch(e){
  ...
  if(isBuy){
    ...
    document.getElementById('swapPriceVal').textContent =
      `1 MON ≈ ${(monUsd/price).toLocaleString(undefined,{maximumFractionDigits:0})} BOB (est.)`;
  } else {
    out = (amtIn * price) / monUsd;
    document.getElementById('swapPriceVal').textContent =
      `1 BOB ≈ ${(price/monUsd).toFixed(6)} MON (est.)`;
  }
...
async function execNativeSwap(){
  ...
  try{
    ...
    if(isBuy){
      ...
      // Step 1: BOB approve → Router
      swapSetStatus('Step 1/2: Approving BOB...','');
    ...
    if(execBtn){
      ...
      execBtn.textContent = currentSwapSide==='buy' ? '▲ BUY BOB' : '▼ SELL BOB';
    }
  }
}
...
async function renderHolderList(){
  ...
  if(myOutOfList){
    ...
    html += `<div style="margin-top:8px;padding:10px 14px;background:rgba(192,132,252,0.08);border:1px solid var(--accent);border-radius:10px;font-size:12px;display:flex;align-items:center;gap:8px">
      ...
      <span style="margin-left:auto;font-family:'Share Tech Mono',monospace">${myAmtM} BOB</span>
    </div>`;
...
function renderTierTable(){
  const content = document.getElementById('holderTabContent');
  const TIERS = [
    {min:100000000, label:'BOB GOD',              badge:'👑 GOD',     cls:'r1', color:'#ffd700'},
    {min:10000000,  label:'Dragon Overlord',        badge:'🐉 DRAGON',  cls:'r2', color:'#e5e7eb'},
    {min:1000000,   label:'BOB Emperor',           badge:'👸 EMPEROR', cls:'r3', color:'#f97316'},
    ...
    {min:0,         label:'ZeroBOB Ghost',          badge:'👻 GHOST',   cls:'r10',color:'#6b7280'},
  ];
  ...
  TIERS.forEach(t => {
    ...
    html += `<div class="tier-row" style="${isMyTier?'border-color:'+t.color+';background:rgba(192,132,252,0.1)':''}">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:10px;color:var(--muted);font-family:'Share Tech Mono',monospace">${minStr}+ BOB</div>
      </div>
  ...
  <div style="margin-top:12px;padding:10px 12px;background:rgba(192,132,252,0.06);border:1px solid var(--border);border-radius:10px;font-size:11px;color:var(--muted);line-height:1.7">
    ${wallet
      ? `💜 Your balance: <b style="color:var(--accent)">${myBal.toLocaleString()} BOB</b> → <b class="${myTier.cls}">${myTier.label}</b>`
...
async function connectWallet(name){
  ...
  try{
    ...
    wallet={addr,bal:Math.floor(bal),name};bobBalance=wallet.bal;
    // MON 잔고도 로드
    try{
    ...
    getHolderRank(addr).then(holderRank => {
      ...
      // 시스템 웰컴 메시지 (BOB Terminal 봇처럼)
      setTimeout(() => {
        const welcomes = nick ? [
          `👋 Welcome back, <b>${nick}</b>! Great to see you 🟣`,
          `🎉 <b>${nick}</b> has entered the BOB Terminal!`,
          `🟣 Hey <b>${nick}</b>! BOB to the moon 🚀`,
        ] : [
          `👋 Welcome to BOB Terminal! Set a nickname to stand out 🟣`,
          `🎉 New trader joined! Connect and set your nickname ✏️`,
          `🟣 Welcome! You're now live on BOB Terminal 🚀`,
        ];
        const w = welcomes[Math.floor(Math.random()*welcomes.length)];
        const isDev2 = addr.toLowerCase() === DEV_WALLET.toLowerCase();
        const devMsg = isDev2 ? `🛠️ <b>BOB Terminal DEV</b> has entered the building! 👑` : null;
        ...
        div.innerHTML = `
          <div class="msg-meta">
            ...
            <span style="font-weight:700;color:var(--accent);font-size:11px">BOB Terminal</span>
    ...
    // 체스 초대 구독 활성화 (지갑 연결 후)
    if(typeof _subscribeToChessInvites === 'function') _subscribeToChessInvites();
...
function showTradeFloat(isBuy, usdValue, bobAmount, monAmount){
  ...
  if(isWhale){
    ...
    wrap.innerHTML = `
      ...
      <div class="trade-float-amount">${Math.floor(bobAmount).toLocaleString()} BOB</div>`;
  } else {
    // 10K~100K MON: 매수=🚀 매도=💀
...
function loadCustomTiersFromStorage(){
  try { devCustomTiers = JSON.parse(localStorage.getItem('bob_custom_tiers')||'{}'); } catch(e){ devCustomTiers={}; }
}
function saveCustomTiersToStorage(){
  localStorage.setItem('bob_custom_tiers', JSON.stringify(devCustomTiers));
}
...
function devApplyFees(){
  ...
  if(shoutDisp) shoutDisp.textContent = '💜 ' + sc.toLocaleString() + ' BOB';
  const nickDisp = document.getElementById('nickCostDisplay');
  if(nickDisp) nickDisp.textContent = '💜 ' + nc.toLocaleString() + ' BOB';
  alert('✅ Fees updated!\nNickname: ' + nc.toLocaleString() + ' BOB\nShout: ' + sc.toLocaleString() + ' BOB');
}
...
function saveShoutsToStorage(){
  try{ localStorage.setItem('bob_shouts', JSON.stringify(pinnedShouts)); }catch(e){}
}

function loadShoutsFromStorage(){
  try{
    const saved = localStorage.getItem('bob_shouts');
    if(!saved) return;
...
async function doShout(){
  ...
  if(!isDev && wallet.bal<SHOUT_COST){
    alert('You need '+SHOUT_COST.toLocaleString()+' BOB to shout!\nBalance: '+wallet.bal.toLocaleString()+' BOB');
    return;
  }
  ...
  if(!isDev){
    ...
    wallet.bal-=SHOUT_COST;bobBalance=wallet.bal;
  }

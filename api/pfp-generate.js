...
async function _handler(req, res) {
  ...
  if (action === 'generate') {
    ...
    const FACE_LOCK = 'CHOG\'s face is a large smooth oval/round shape — NOT a cat face, NOT a mouse face. Plain round chibi oval with NO animal ears, NO snout, NO whiskers. FACE SHAPE IS ABSOLUTELY LOCKED to the base image.';

    const STYLE_REF = 'The image labeled example.jpg shows the EXACT TARGET ART STYLE — study it carefully. Every output must match example.jpg\'s style: thick bold black outlines on every edge, completely flat solid colors with ZERO gradients, ZERO shading, ZERO soft blending, ZERO watercolor or painted texture. Colors are pure and flat like stickers. This style is NON-NEGOTIABLE.';

    const ART_STYLE = chogStyle === '2'
      ? `⚠ ART STYLE IS LOCKED. ${STYLE_REF} Maintain CHOG's exact style: thick bold black outlines, flat solid colors, large circular anime eyes, cute chibi proportions, spiky hair. ${FACE_LOCK} NOSE (CRITICAL): small dark nose mark between eyes and mouth — always visible. MOUTH IS REPLACED BY CIGARETTE (LOCKED): thick lit cigarette/cigar at corner of mouth — ALWAYS present. Do NOT adopt the reference image's art style, shading, or face shape. Only transplant accessories/outfit/hair onto CHOG.`
      : `⚠ ART STYLE IS LOCKED. ${STYLE_REF} Maintain CHOG's exact style: thick bold black outlines, flat solid colors, large circular anime eyes, cute chibi proportions, spiky hair. ${FACE_LOCK} NOSE (CRITICAL): small dark nose mark between eyes and mouth — always visible. Mouth = one thin slightly curved line, always present. Do NOT adopt the reference image's art style, shading, or face shape. Only transplant accessories/outfit/hair onto CHOG.`;

    const COMPOSITION = 'COMPOSITION: preserve EXACTLY the composition, framing, camera distance, character scale, and positioning of the provided base CHOG image — do NOT change the zoom level, angle, or how much of the character is visible. Same shot as the base image, only hair/accessories/outfit changed.';

...
async function _handler(req, res) {
  ...
  if (action === 'generate') {
    ...
    const editPrompt = `The first image is the CHOG base — match its art style (thick black outlines, flat solid colors, no gradients, no shading) and composition (extreme close-up face, left-heavy framing, head and spikes bleeding off edges) exactly. CRITICAL: The face must remain the same size as the base — do not zoom out or scale down the face to fit accessories or hair. Accessories must be placed on top of the existing head size, not used as a reason to shrink the face. Its framing, body pose, and arm position are absolutely locked. The face must look exactly like the CHOG character in the first image — large simple black circle eyes with white highlight, small pink dot nose, chubby round face, pink oval blush marks. Do NOT apply the reference character face features, eye style, or face shape. The second image is the style reference — apply only: (1) ${hairInstruction}, (2) outfit color/accessories in the very bottom zone only, (3) skin color if different, (4) mouth expression in the tight mouth zone only — do not let the mouth expand beyond the zone. Do not change the eyes, pupils, or nose. Do not copy the reference face, composition, pose, art style, or background. Only modify the unmasked zones.${eyelashPart}${cigarettePart}${extraPart ? ' ' + extraPart : ''}`;

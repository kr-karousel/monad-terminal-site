...
async function _handler(req, res) {
  ...
  if (action === 'generate') {
    ...
    if (chogStyle !== '2') editZones.push([0.22, 0.63, 0.72, 0.72]); // mouth zone (tighter, below nose)
    ...
    const eyelashPart = (semantics.eyelash === true || semantics.eyelash === 'true')
      ...
      : '';
    const editPrompt = `The first image is the CHOG base — follow its art style (thick black outlines, flat solid colors, no gradients), composition (extreme close-up face, left-heavy framing, head and spikes bleeding off edges), and body pose (including arm position) exactly. The spike shape, count, and direction must be pixel-identical to the base — do not soften, round, or alter the spikes in any way. The face outline and eye shape are absolutely locked — do not change them. The second image is the style reference — apply only: (1) hair color (spike shape stays identical to base), (2) outfit and accessories, (3) skin color if different, (4) mouth expression. Do not change the eyes, pupils, eye shape, nose, or face outline. Do not draw hair onto the face or forehead. Do not copy the reference composition, pose, or background. Only modify the unmasked zones.${eyelashPart}${cigarettePart}${extraPart ? ' ' + extraPart : ''}`;

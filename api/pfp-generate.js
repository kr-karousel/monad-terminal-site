...
async function _handler(req, res) {
  ...
  if (action === 'generate') {
    ...
    const IDENTITY_LOCK = 'DO NOT REDRAW the character. The existing character image must remain visually identical — do NOT change: face shape, head shape, spike silhouette, spike count/direction, hair shape, eye placement, face proportions, framing, crop, or background color. The face silhouette must exactly match the base image — the lower jaw and cheeks must be round and prominent, especially the right cheek (right side of image) which bulges downward and outward. The face is partially cut off by the frame edges — preserve this exactly. Do NOT generate a new version of the character. Hair shape and spike pattern must stay identical to the base image — ONLY the hair color may change. The face (eyes, nose, mouth, cheeks, expression, all facial features) must be taken EXACTLY from the base image — do NOT apply any facial feature from the reference image. Only ADD accessories/outfit into the unmasked zones.';

...
async function _handler(req, res) {
  ...
  if (action === 'generate') {
    ...
    const ART_STYLE = '⚠ STYLE TARGET: the example images provided show EXACTLY what the output must look like — study them carefully and match their visual style precisely. Key qualities from the examples: jet-black thick outlines, 100% flat solid color fills (zero gradients, zero shading, zero texture), bold clean linework with sharp corners, bright solid blue background, chibi proportions. The output must be indistinguishable in style from those example images. The user\'s reference image provides ONLY the hair design and outfit concept — redraw those elements in the exact same flat-ink style as the examples. Do NOT carry over any shading, texture, or line quality from the user\'s reference image.';

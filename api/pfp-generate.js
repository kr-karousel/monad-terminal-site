...
async function _handler(req, res) {
  ...
  if (action === 'generate') {
    ...
    const COMPOSITION = 'COMPOSITION: SCALE IS ABSOLUTELY FIXED — reproduce the character at EXACTLY the same size as in the provided base CHOG image. Do NOT enlarge, zoom in, or scale up the character in any way. The character scale in the output must match the base image precisely. Face occupies the LEFT 55% of the image horizontally. RIGHT frame edge slices through the face just past the right eye. VERTICAL FRAMING: eyes at approximately 45% from top, chin at approximately 75% from top, upper body/outfit visible in bottom 25%. The top of the head bleeds OFF the top edge — not fully visible. Do NOT center. Accessories may bleed off any edge.';

...
async function _handler(req, res) {
  ...
  if (action === 'generate') {
    ...
    const OUTLINE_STYLE = 'OUTLINE STYLE IS LOCKED — CHOG\'s outlines are thick bold black lines that are NOT perfectly smooth or geometric; they have subtle hand-drawn irregularity and slight wobbliness, as if drawn with a thick marker by hand. Preserve this imperfect, slightly jagged quality exactly — do NOT smooth out or make the outlines perfectly round/geometric.';
    ...
    const COMPOSITION = 'COMPOSITION: extreme close-up portrait — the face is VERY LARGE and dominates the frame. Face occupies the LEFT 55% of the image horizontally. RIGHT frame edge slices through the face just past the right eye. VERTICAL FRAMING IS STRICT: eyes at 40% from top, chin at 85% from top, only a sliver of upper body visible at the very bottom (bottom 10-15% only). The top of the head bleeds FAR OFF the top edge — only the lower half of the head is visible. Do NOT zoom out. Do NOT show more than a sliver of body. Do NOT center. Accessories may bleed off any edge.';

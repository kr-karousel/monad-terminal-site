...
async function _handler(req, res) {
  ...
  if (action === 'generate') {
    ...
    const IDENTITY_LOCK = 'DO NOT REDRAW the character. The existing character image must remain visually identical — do NOT change: face shape, head shape, spike silhouette, spike count/direction, hair shape, eye placement, face proportions, framing, crop, or background color. The face is partially cut off by the frame edges — preserve this exactly. Do NOT generate a new version of the character. Hair shape and spike pattern must stay identical to the base image — ONLY the hair color may change. Only ADD accessories/outfit into the unmasked zones.';

    const ART_STYLE = chogStyle === '2'
      ? 'ART STYLE: thick bold black outlines, PURE FLAT SOLID COLORS (zero gradients, zero shading, zero blending — hard flat fills only), large circular anime eyes, cute chibi proportions. NOSE: small dark dot always visible at center of face — do NOT omit. Mouth and cigarette are already in the base image — do NOT redraw, do NOT add tongue, do NOT replace.'

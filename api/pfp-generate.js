...
async function _handler(req, res) {
  ...
  if (action === 'generate') {
    ...
    const ART_STYLE = '⚠ TREAT THE BASE CHOG IMAGE AS THE EXACT TEMPLATE. The output must look like the base image itself — same jet-black outline thickness, same flat solid color fills, same line weight on every edge, same overall visual feel. You are ONLY swapping the hair color/shape and the outfit in the edit zones. Every outline, every stroke, every proportion outside the edit zones must be pixel-identical in style to the base. The new hair and outfit must use the SAME outline thickness and flat-fill style as the base — no gradients, no soft edges, no shading, no texture. The reference image provides ONLY the hair style and outfit design to draw in CHOG\'s existing linework style. Do NOT let the reference image influence line quality, shading, or proportions in any way.';

...
async function _handler(req, res) {
  ...
  if (action === 'generate') {
    ...
    const editPrompt = `The first image is the CHOG base — its composition and framing are FIXED and must not change at all: the face size, body size, camera angle, and crop are identical to the base. Do not zoom out, do not center, do not reframe. Follow its art style (thick black outlines, flat solid colors, no gradients). The second image is the style reference — apply the following onto the CHOG base: (1) hair color and style — if the reference has spiky hair keep CHOG spikes, otherwise reinterpret the hair style in CHOG's flat cartoon art style; (2) eye shape and expression — redraw to match the reference, drawn in CHOG's flat cartoon art style (bold outlines, solid fills); (3) mouth expression — match the reference, drawn in CHOG's art style; (4) outfit and accessories — dress the CHOG body naturally with the reference outfit. Keep the nose as a tiny dark dot. Do not copy the reference composition or background.${cigarettePart}${extraPart ? ' ' + extraPart : ''}`;

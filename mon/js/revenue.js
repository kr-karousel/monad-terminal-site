// ═══════════════════════════════════════
//  MONAD TERMINAL - REVENUE DASHBOARD
//  수익은 MON 네이티브 토큰으로 분배
// ═══════════════════════════════════════

const CONTRIB_KEY = 'mon_contrib_v1';
const REV_TOP_N   = 20;
...
// ── DEV WALLET MON 잔고 (네이티브) ────────────────
async function fetchDevMonBalance() {
  try {
    const hex = await fetch(MONAD_RPC, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ jsonrpc: '2.0', id: 1, method: 'eth_getBalance', params: [DEV_WALLET, 'latest'] })
...
async function renderRevenueModal() {
  ...
  if (balEl)  balEl.textContent  = devBal.toFixed(4) + ' MON';
  if (poolEl) poolEl.textContent = poolAmt.toFixed(4) + ' MON';
...
function renderContribTable(devBal, serverContribs) {
  ...
  if (serverContribs && serverContribs.length > 0) {
    all = serverContribs.map(row => ({
      addr: row.address, nickCount: row.nick_count || 0, shoutCount: row.shout_count || 0,
      ...calcUserPoints(row)
    }));
  } else {
    const db = getContribDB();
    all = Object.entries(db).map(([addr, entry]) => ({
      addr, nickCount: entry.nickCount || 0, shoutCount: entry.shoutCount || 0,
  ...
  top.forEach((e, idx) => {
    ...
    rows += `<tr class="${rowCls}">
      ...
      <td class="rev-td rev-tr rev-est">${est != null ? '~' + est.toFixed(2) + ' MON' : '—'}</td>
    </tr>`;
  });
...
async function devResetPoints() {
  ...
  if (_sbReady()) {
    ...
    } catch(e) { alert('Supabase reset failed: ' + e.message); }
  }
  renderRevenueModal();
}
...
async function devDistributeRevenue() {
  ...
  if (!devBal || devBal <= 0) { alert('Dev wallet MON balance is 0.'); return; }

  const serverContribs = await loadContribsFromServer();
  let allEntries;
  if (serverContribs && serverContribs.length > 0) {
    allEntries = serverContribs.map(row => ({ addr: row.address, ...calcUserPoints(row) }));
  } else {
    const db = getContribDB();
    allEntries = Object.entries(db).map(([addr, entry]) => ({ addr, ...calcUserPoints(entry) }));
  }
  ...
  if (!top.length) { alert('No contributors found.'); return; }
  ...
  const preview = top.map((e, i) => {
    ...
    const amt  = (poolAmt * e.total / totalPts).toFixed(4);
    const pct  = (e.total / totalPts * 100).toFixed(1);
    return `#${i + 1} ${nick} → ${amt} MON (${pct}%)`;
  }).join('\n');

  if (!confirm(`Distribute ${poolAmt.toFixed(4)} MON to ${top.length} contributors?\n\n${preview}\n\nThis will send ${top.length} native MON transactions.`)) return;

  await ensureMonadChain(window.ethereum);

  let success = 0, failed = 0;
  for (const e of top) {
    const amt = poolAmt * e.total / totalPts;
    if (amt < 0.0001) continue;
    try {
      const valueWei = BigInt(Math.floor(amt * 1e18));
      await window.ethereum.request({
        method: 'eth_sendTransaction',
        params: [{ from: wallet.addr, to: e.addr, value: '0x' + valueWei.toString(16), gas: '0x5208' }]
      });
      success++;
    } catch(err) {

...
// ═══════════════════════════════════════
(function(){
  const STORAGE_KEY = 'monTutorialSeen_v1';
  const ACCENT = '#a78bfa';

  const STEPS = [
    {
      target: null,
      title: '👋 Welcome to MON Terminal',
      text: 'Real-time trading hub for MON holders. Quick 30-second tour of the main features.',
      pos: 'center'
    },
    {
      target: '.nav-right',
    ...
    },
    {
      ...
      title: '🔔 Price Alerts',
      text: 'Click the bell to set a target price. Get email notifications when MON hits your target — even when this tab is closed.',
      pos: 'bottom'
    },
    {
      target: '#chatList',
      title: '💬 Live Chat',
      text: 'Connect your wallet to chat with other MON holders in real-time. Send stickers, share vibes, build the community.',
      pos: 'top'
    },
    {
      target: '.shout-section',
      title: '📢 SHOUT',
      text: 'Pay MON to broadcast a pinned message visible on the chart for everyone. Up to 3 shouts stay pinned at a time.',
      pos: 'top'
    }
  ];
  ...
  function injectStyle(){
    ...
    const css = `
      .tut-backdrop{position:fixed;inset:0;background:rgba(7,6,18,0.78);z-index:99998;pointer-events:auto;backdrop-filter:blur(2px)}
      .tut-spotlight{position:fixed;border-radius:12px;box-shadow:0 0 0 9999px rgba(7,6,18,0.85),0 0 0 3px ${ACCENT},0 0 32px rgba(167,139,250,0.5);z-index:99998;pointer-events:none;transition:all .3s ease}
      .tut-tooltip{position:fixed;max-width:340px;background:linear-gradient(180deg,rgba(20,15,42,0.99),rgba(7,6,18,0.99));border:1px solid rgba(167,139,250,0.45);border-radius:14px;padding:18px 20px;color:#ede8f5;font-size:13px;line-height:1.55;z-index:99999;box-shadow:0 16px 40px rgba(0,0,0,0.6);pointer-events:auto;animation:tutFade .25s ease}
      ...
      .tut-btn-primary{background:linear-gradient(135deg,#a78bfa,#7c3aed);color:#fff;box-shadow:0 4px 12px rgba(124,58,237,0.4)}

...
function loadPriceAlerts(){
  try{ priceAlerts = JSON.parse(localStorage.getItem('mon_price_alerts')||'[]'); }
  catch(e){ priceAlerts = []; }
  alertEmail = localStorage.getItem('mon_alert_email') || '';
  updateAlertBellState();
}

function savePriceAlerts(){
  localStorage.setItem('mon_price_alerts', JSON.stringify(priceAlerts));
}
...
async function saveAlertEmail(){
  ...
  alertEmail = val;
  localStorage.setItem('mon_alert_email', val);
...
function openPriceAlertModal(){
  ...
  if(inp && livePrice && !inp.value) inp.value = livePrice.toFixed(4);
  ...
  el.innerHTML = priceAlerts.map(a => `
    <div style="display:flex;align-items:center;gap:6px;background:rgba(255,255,255,.04);border-radius:7px;padding:6px 8px;margin-bottom:4px${a.triggered ? ';opacity:0.4' : ''}">
      ...
      <span style="font-size:11px;flex:1;line-height:1.4">
        ...
        <b style="font-family:'Share Tech Mono',monospace;color:var(--accent)">$${a.price.toFixed(4)}</b>
...
function _fireAlert(alert, currentPrice){
  ...
  const msg = `MON hit $${alert.price.toFixed(4)}\nNow: $${currentPrice.toFixed(4)}`;

  // Browser notification
  if(alertNotifGranted && 'Notification' in window){
    try{ new Notification('🎯 MON Price Alert', { body: msg, icon: '/mon/img/mon_logo.png' }); }
    catch(e){}
  }
...
function _showAlertToast(type, targetPrice, currentPrice){
  ...
  toast.innerHTML = `
    ...
    <div style="font-size:12px;font-weight:700;color:var(--accent);letter-spacing:.5px">
      MON HIT $${targetPrice.toFixed(4)}
    </div>
    <div style="font-size:10px;color:var(--muted);margin-top:3px">
      Now: <b style="font-family:'Share Tech Mono',monospace;color:var(--text)">$${currentPrice.toFixed(4)}</b>
    </div>`;

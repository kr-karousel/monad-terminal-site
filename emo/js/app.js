...
var NICK_BANNED  = [
  ...
  'official','support','scam','emo','emo_official','nadfun',
...
// ═══════════════════════════════════════
const RANKS=[
  {min:100000000,label:'EMO GOD',badge:'👑 GOD',cls:'r1'},
  {min:10000000,label:'Dragon Overlord',badge:'🐉 DRAGON',cls:'r2'},
  {min:1000000,label:'EMO Emperor',badge:'👸 EMPEROR',cls:'r3'},
  ...
  {min:0,label:'ZeroEMO Ghost',badge:'👻 GHOST',cls:'r10'},
];
function getRank(b, addr){
  // DEV 지갑은 항상 최상위 계급으로 표시
  if(addr && addr.toLowerCase() === DEV_WALLET.toLowerCase()){
    return {min:0,label:'EMO Terminal DEV',badge:'🛠️ DEV',cls:'r1',isDev:true};
  }
  // 커스텀 Tier 라벨 확인
  if(addr){
...
function saveCustomTiersToStorage(){
  localStorage.setItem('emo_custom_tiers', JSON.stringify(devCustomTiers));
}
...
function devApplyFees(){
  ...
  if(shoutDisp) shoutDisp.textContent = '🖤 ' + sc.toLocaleString() + ' EMO';
  const nickDisp = document.getElementById('nickCostDisplay');
  if(nickDisp) nickDisp.textContent = '🖤 ' + nc.toLocaleString() + ' EMO';
  alert('✅ Fees updated!\nNickname: ' + nc.toLocaleString() + ' EMO\nShout: ' + sc.toLocaleString() + ' EMO');
}
...
function saveShoutsToStorage(){
  try{ localStorage.setItem('emo_shouts', JSON.stringify(pinnedShouts)); }catch(e){}
}

function loadShoutsFromStorage(){
  try{
    const saved = localStorage.getItem('emo_shouts');
    if(!saved) return;
